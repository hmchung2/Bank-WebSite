...
<link
	href="<%=request.getContextPath()%>/assets/vendor/bootstrap/css/bootstrap.css"
	rel="stylesheet">
<link
...
<script>
	...
	function loginCheck() {	
			...
			if( $('#login_pw2').val() == "" ){
				msg = "새비밀번호는 작성 바랍니다."			
			}else if( $('#login_pw2').val() != $('#login_pw3').val() ) {
				msg = "비밀번호를 정확히 재입력 바랍니다."	
			}else if( $('#login_pw2').val() == ${userVO.pwd} ){
				msg = "현재 사용하는 비밀번호랑 일치 합니다."			
			}else if( $('#login_pw2').val().length < 5  ){
				msg = "비밀번호는 최소 5글자 이상 필요합니다."
				
			}else if($('#login_pw').val() != ${userVO.pwd}  ){    
				msg = "비밀번호가 올바르지 않습니다."			
			} else{
				return true
			}
		...
		<section class="inner-page">
			<div class="container">
				...
				<div class="d-flex justify-content-between align-items-center">
					...
					<h2 style="margin: auto; margin-top: 30px; margin-bottom: 20px;">비밀번호 수정</h2>
				</div>

				<main class="form-signin">
					<form id="my-login-form"
						action="<%=request.getContextPath() %>/member/userUpdateInfoProcess.do" method="post" style="width: 50%; margin: 0px auto" onsubmit="return loginCheck()">

						<input type="text" hidden="true" id="destination"
							name="destinationParam" value="${destination }">
						...
						<div class="form-group input-group" id="field2">
							<div class="input-group-prepend" id="field2">
								<span class="input-group-text"> <i
									class="fa fa-lock fa-sm"></i>
								</span>
							</div>
							<input autocomplete="anyrandomstring" name="login_pw"
								class="form-control" placeholder="비밀번호" type="password"
								id="login_pw">
						</div>


						<div class="form-group input-group" id="field3">
							<div class="input-group-prepend" id="field3">
								<span class="input-group-text"> <i
									class="fa fa-lock fa-sm"></i>
								</span>
							</div>
							<input autocomplete="anyrandomstring" name="login_pw2"
								class="form-control" placeholder="새비밀번호" type="password"
								id="login_pw2">
						</div>


						<div class="form-group input-group" id="field4">
							<div class="input-group-prepend" id="field4">
								<span class="input-group-text"> <i
									class="fa fa-lock fa-sm"></i>
								</span>
							</div>
							<input autocomplete="anyrandomstring" name="login_pw3"
								class="form-control" placeholder="새비밀번호 확인" type="password"
								id="login_pw3">
						</div>
						...
						<input hidden="true" id="kakao_id"> <input hidden="true"
							id="kakao_email">


						<div class="form-group" style="width: 100%;">
							<button type="submit" class="btn btn-primary btn-block"
								id="loginbtn" style="width: 50%;">수정하기</button>
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<img
							src="<%=request.getContextPath()%>/assets/img/illustration-6.svg"
